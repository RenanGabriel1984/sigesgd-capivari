...
export default function Entries() {
  ...
  const handleSave = async () => {
    if (!productId || quantity <= 0) { toast.error("Selecione um produto e informe uma quantidade válida"); return; }
    try {
  ...
  return (
    <AppShell>
            ...
            <div className="space-y-3">
              {recentEntries.map((m) => (
                <div key={m._id} className="flex items-center justify-between py-2 border-b border-border/30 last:border-0">
                  <div><p className="font-medium text-sm">{m.product?.name ?? "Produto"}</p><p className="text-xs text-muted-foreground">{m.user?.name ?? "Usuário"}{m.supplier ? ` • ${m.supplier.legalName}` : ""}{m.documentNumber ? ` • Doc: ${m.documentNumber}` : ""}</p></div>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-md"><DialogHeader><DialogTitle>Nova Entrada</DialogTitle></DialogHeader>
          <div className="space-y-4 py-2">
            <div><Label>Produto *</Label><Select value={productId} onValueChange={setProductId}><SelectTrigger><SelectValue placeholder="Selecionar produto" /></SelectTrigger><SelectContent>{products?.map((p) => (<SelectItem key={p._id} value={p._id}>{p.name}</SelectItem>))}</SelectContent></Select></div>
            <div><Label>Quantidade *</Label><Input type="number" min="1" value={quantity} onChange={(e) => setQuantity(Number(e.target.value))} /></div>
            <div><Label>Fornecedor</Label><Select value={supplierId} onValueChange={setSupplierId}><SelectTrigger><SelectValue placeholder="Nenhum" /></SelectTrigger><SelectContent>{suppliers?.map((s) => (<SelectItem key={s._id} value={s._id}>{s.legalName}</SelectItem>))}</SelectContent></Select></div>
