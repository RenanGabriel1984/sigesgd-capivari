...
export default function Suppliers() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingId) {
        await updateSupplier({ id: editingId as any, ...data });
      ...
      <div className="space-y-6 max-w-7xl mx-auto">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground">Gerenciar fornecedores</p>
          </div>
        ...
        <div className="relative">
          ...
          <Input placeholder="Buscar por razão social, nome fantasia ou CNPJ..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-9 max-w-md" />
        </div>
                ...
                <TableBody>
                  {filtered?.map((s) => (
                    <TableRow key={s._id}>
                      ...
                      <TableCell className="text-muted-foreground">{s.tradeName ?? "—"}</TableCell>
