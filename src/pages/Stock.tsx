    ...
    <AppShell>
      <div className="space-y-6 max-w-7xl mx-auto">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Visão geral do estoque de produtos</p>
        </div>

        <div className="relative">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
          <Input placeholder="Buscar produto..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-9 max-w-md" />
        </div>
                  ...
                  {filtered?.map((p) => {
                    ...
                    return (
                      <TableRow key={p._id}>
                        ...
                        <TableCell className="text-center font-mono text-amber-600">{reserved}</TableCell>
