...
function OrgTreeNode({ org, allOrgs, onEdit, level = 0 }: { org: any; allOrgs: any[]; onEdit: (org: any) => void; level?: number; }) {
  const [expanded, setExpanded] = useState(level < 1);
  const grandchildren = allOrgs.filter((o) => o.parentId === org._id);
  return (
    <div>
      <div className="flex items-center gap-2 py-2 px-3 rounded-lg hover:bg-muted/50 transition-colors group" style={{ paddingLeft: `${level * 24 + 12}px` }}>
        {grandchildren.length > 0 ? (<Button variant="ghost" size="icon" className="h-6 w-6 shrink-0" onClick={() => setExpanded(!expanded)}>{expanded ? <ChevronDown className="h-3 w-3" /> : <ChevronRight className="h-3 w-3" />}</Button>) : (<div className="w-6 shrink-0" />)}
        <Building2 className="h-4 w-4 text-muted-foreground shrink-0" />
        <div className="flex-1 min-w-0"><span className="text-sm font-medium truncate">{org.name}</span><Badge variant="secondary" className="ml-2 text-[10px]">{ORG_TYPE_LABELS[org.type as OrgType]}</Badge></div>
        <Badge variant={org.active ? "default" : "secondary"} className="text-[10px] shrink-0">{org.active ? "Ativo" : "Inativo"}</Badge>
        <Button variant="ghost" size="icon" className="h-7 w-7 opacity-0 group-hover:opacity-100 shrink-0" onClick={() => onEdit(org)}><Pencil className="h-3.5 w-3.5" /></Button>
      </div>
      {expanded && grandchildren.map((child) => (<OrgTreeNode key={child._id} org={child} allOrgs={allOrgs} onEdit={onEdit} level={level + 1} />))}
    </div>
  );
}
    ...
    <AppShell>
      <div className="space-y-6 max-w-5xl mx-auto">
        ...
        <Card className="border-border/50"><CardContent className="p-2">
          {rootOrgs.length === 0 ? (<div className="text-center py-12"><Building2 className="h-8 w-8 mx-auto text-muted-foreground mb-2" /><p className="text-muted-foreground">Nenhuma unidade organizacional cadastrada</p></div>) : (rootOrgs.map((org) => (<OrgTreeNode key={org._id} org={org} allOrgs={orgData?.orgs ?? []} onEdit={openEdit} />)))}
        </CardContent></Card>
      </div>
