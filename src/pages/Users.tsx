  ...
  const handleSave = async () => {
    ...
    try {
      await updateUser({
        userId: editUser._id,
        role,
        organizationId: orgId || undefined as any,
        active,
      });
    ...
    <AppShell>
      <div className="space-y-6 max-w-7xl mx-auto">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Gerenciar perfis e permissões de usuários</p>
        </div>
                ...
                <TableBody>
                  {filtered?.map((u) => {
                    const orgName = u.organization
                      ? orgs?.orgs.find((o) => o._id === u.organizationId)?.name
                      : null;
                    return (
                      <TableRow key={u._id}>
